...
public class DESUtils {
	...
	private static String byte2hex(byte[] digest) {
		StringBuilder sbTogether = new StringBuilder();
		String stmp = "";
		for (int n = 0; n < digest.length; n++) {
			stmp = Integer.toHexString(digest[n] & 0XFF);
			if (stmp.length() == 1) {
				sbTogether.append("0").append(stmp);
			} else {
				sbTogether.append(stmp);
			}
		}
		return sbTogether.toString().toUpperCase();
	}
	...
	public static void main(String[] args) {
		...
		System.out.println(decrypt("0540F0A41839E73792D56C93976C4FB7DB1E3AD2A88066076C2944683B49C700AD6A88B4FA37833D", secretKey));
	}
}
