...
import com.cn.jdbc.MerchantJdbcClient;
 ...
 * 2017年1月10日下午3:11:55
 */
public class MerchantSelfServiceServlet extends HttpServlet {

	/**
	 * 
	 */
	private static final long serialVersionUID = -6468510089135469187L;
	...
	/*****/
    private MerchantJdbcClient jdbcClient;
	...
	public void init(ServletConfig config)throws ServletException{
		...
		this.jdbcClient = new MerchantJdbcClient(dataBaseFile);
	}
	...
	public void service(HttpServletRequest req, HttpServletResponse resp)
		...
		}else if("loginFromBusiness".equals(marked)){//从业务系统跳转到商助系统
	...
	private void beforeLogin(HttpServletRequest request, HttpServletResponse response,Cookie ticket)
		...
		if(ticket == null) {//名为SSOID的cookie为空直接跳转至登录页面
			LogHelper.info("^^^^^^商助系统^^^^^^^跳转到首页gotoURL:"+urlNew);
			response.sendRedirect(urlNew.toString());
		}else {//名为SSOID的cookie不为空
			LogHelper.info("^^^^^商助系统^^^^^^^跳转到首页setCookieURL:"+setCookieURL);
	...
	private void checkUpTicket(HttpServletRequest request, HttpServletResponse response,
		...
		LogHelper.info("^^^^^^商助系统^^^^^^^校验cookie有效性　data:"+data);
	...
	private void doLogout(HttpServletRequest request, HttpServletResponse response,
		...
		if(jsonStr != null){
			...
			if(!Utils.isNullOrEmpty(data.get("ssoCookie"))){
				...
				String decodedTicket = DESUtils.decrypt(encodedTicket, this.secretKey);
				LogHelper.info("^^^^^商助系统^^^^^^^退出系统decodedTicket:"+decodedTicket);
				String ticketTime = this.redisClient.getTicket(decodedTicket);
				if(ticketTime != null){
	...
	private void doLogin(HttpServletRequest request, HttpServletResponse 
		...
		LogHelper.info("^^^^^^商助系统^^^^^^^登录系统data:"+data);
		...
		if(!jsonStr.containsKey("username") || !jsonStr.containsKey("password")){
			...
			if(jsonStr.containsKey("keepLoginForWeek")){
			...
			if(result.isEmpty()){
			...
			}else if(!"1".equals(result.get("merchantState"))){
				json.put("rspCode", "004");
				json.put("rspDesc", "商户不存在!!!");
				...
				json.put("rspCode", "000");
				json.put("rspDesc", "");
				LogHelper.info("^^^^^商助系统^^^^^^^登录成功data:"+data);
			}
		    /****/
			if(cookie != null){
	...
	private void loginFromBusiness(HttpServletRequest request, HttpServletResponse 
		...
		LogHelper.info("^^^^^^业务系统跳转到商助系统^^^^^^^登录系统data:"+data);
		Map<String, Object> jsonStr = parseSourceJson(data);
		JSONObject json = new JSONObject();
		if(!jsonStr.containsKey("username") || !jsonStr.containsKey("merchantId")){
			json.put("rspCode", "005");
			json.put("rspDesc", "用户名或商户ID不能为空!!!");
		}else{
			Map<String, Object> result = this.jdbcClient.
					queryAdminByMerchantId(jsonStr.get("merchantId").toString());
			Cookie cookie = null;
			if(result.isEmpty()){
				json.put("rspCode", "003");
				json.put("rspDesc", "商助用户不存在,请创建商助用户!!!");
			}else{
				result.put("mer_userName", "sys_"+jsonStr.get("username"));
				cookie = createSsoCookie(result,null,"0");
				json.put("rspCode", "000");
				json.put("rspDesc", "");
				LogHelper.info("^^^^^^业务系统跳转到商助系统^^^^^^^登录成功data:"+data);
			}
			if(cookie != null){
				response.addCookie(cookie);
				response.addCookie(createUserCookie(result));
	...
	private String obtainLoginPageUrl(HttpServletRequest request){
		...
		urlNew.append("/merchant/login.html");
	 ...
	 */
	 private Cookie createUserCookie(Map<String, Object> result) 
        ...
        JSONObject json = new JSONObject();
        json.put("mer_userName", checkTextIsOrNotNull(result.get("mer_userName")));
    	...
    	json.put("userRole", checkTextIsOrNotNull(result.get("userRole")));
    	json.put("userType", checkTextIsOrNotNull(result.get("userType")));
    	json.put("merchantId", checkTextIsOrNotNull(result.get("merchantId")));
    	json.put("shopName", checkTextIsOrNotNull(result.get("shopName")));
    	json.put("branchCompanyId", checkTextIsOrNotNull(result.get("branchCompanyId")));
    	json.put("branchCompanyName", checkTextIsOrNotNull(result.get("branchCompanyName")));
    	if(result.containsKey("storeId")){
    		json.put("storeId", checkTextIsOrNotNull(result.get("storeId")));
    	}
     ...
     * @throws IOException
     */
	private Cookie createSsoCookie(Map<String, Object> result,Cookie ticket,String 
			keepLoginForWeek) throws IOException{
