...
function openIframe() {
	$(".sidebar-menu").on("click", "a", function() {
		...
		var ifmSysUrl = $(this).attr("data-pageUrl");
		var ifmFullUrl;
		if(ifmSysUrl != undefined && ifmSysUrl != null && ifmSysUrl != '') {
			ifmFullUrl = ifmSysUrl + tabId;
			//判断url中是否有hash值 并截取 例：www.a.com/index.html#abc.html 截取结果 abc
			if (ifmFullUrl.indexOf("#") > 0) {
				tabId = ifmFullUrl.replace(/(.*\#)*([^.]+).*/ig, "$2");
			}
			else
			{	
				//截取url中最后的页面名称 例：www.a.com/index.html#abc.html 截取结果 index
				tabId = ifmFullUrl.replace(/(.*\/)*([^.]+).*/ig, "$2");
			}
		}

		if(tabId != 'undefined' && tabId != 'null' && tabId != '' && tabId != undefined && tabId != null) {
			var loadSign = true;
