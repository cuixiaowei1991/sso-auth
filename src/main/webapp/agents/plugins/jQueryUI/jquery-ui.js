...
(function(factory) {
		...
		_create: function() {
			...
			this._on(this.menu.element, {
				...
				menufocus: function(event, ui) {
					...
					label = item == undefined && ui.item.attr("aria-label") == undefined ? '' : ui.item.attr("aria-label") || item.value;
		...
		_initSource: function() {
			...
			if($.isArray(this.options.source)) {
				...
				this.source = function(request, response) {
					response($.ui.autocomplete.filter(array, request.term, this.options.isMultipe, this.options.fliterName));
				};
	...
	$.extend($.ui.autocomplete, {
		...
		filter: function(array, term, ismultipe, flitername) {
			var matcher = new RegExp($.ui.autocomplete.escapeRegex(term), "i");
			if(ismultipe) {
				if(array.length > 0) {
					if(typeof array[0] === 'object') {
						var arrayMultipe = new Array();
						$.each(array, function(index, entry) {
							if(eval("entry." + flitername).indexOf(term) > -1) {
								arrayMultipe.push(entry);
							}
						});
						return arrayMultipe;
					} else {
